...
Interface::Interface() {
  ...
  media.append({ID::GameBoy,      "Game Boy",       "gb" , false});
  media.append({ID::GameBoyColor, "Game Boy Color", "gbc", false});

  {
    Device device{0, ID::Device, "Super Nintendo Input"};
    this->device.append(device);
  }

  port.append({0, "Adapter", {device[0]}});
}
