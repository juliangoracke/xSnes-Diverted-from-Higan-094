...
void Utility::load() {
  ...
  stateManager->load({pathname[0], "states/states.bsa"}, 1);
...
void Utility::unload() {
  ...
  stateManager->save({pathname[0], "states/states.bsa"}, 1);
...
void Utility::saveState(unsigned slot) {
  ...
  if(s.size() == 0) return;
  directory::create({pathname[0], "states/"});
  if(file::write({pathname[0], "states/state-", slot, ".bsa"}, s.data(), s.size()) == false);
  showMessage({"Saved to slot ", slot});
}

void Utility::loadState(unsigned slot) {
  if(program->active == nullptr) return;
  auto memory = file::read({pathname[0], "states/state-", slot, ".bsa"});
...
string Utility::libraryPath() {
  string path = string::read({configpath(), "xSnes/library.bml"}).strip().ltrim<1>("Path: ").transform("\\", "/");
